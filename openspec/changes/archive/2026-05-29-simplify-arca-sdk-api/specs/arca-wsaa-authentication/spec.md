...
The authentication flow MUST be orchestrated through a centralized internal `WsaaTokenManager` that owns login, in-memory cache, expiration validation, and renewal decisions.
...
- **THEN** the issuance path MUST be executed by `WsaaTokenManager`
...
- **THEN** cache replacement and expiration update MUST be coordinated by `WsaaTokenManager`
