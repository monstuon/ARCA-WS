...
ErpCredentialSnapshot? constanciaCredentials = null;
...
(string? token, string? sign) ApplyConstanciaCredentials()
{
    if (constanciaCredentials is null)
    {
        return (null, null);
    }

    if (constanciaCredentials.Expiration <= DateTimeOffset.UtcNow.AddSeconds(30))
    {
        logger.LogWarning(
            "Credenciales Constancia vencidas o por vencer (Exp={Expiration:o}); se omiten para permitir fallback WSAA.",
            constanciaCredentials.Expiration);
        constanciaCredentials = null;
        return (null, null);
    }

    return (constanciaCredentials.Token, constanciaCredentials.Sign);
}
...
void CaptureConstanciaCredentialsFromResult(string scenario, PersonaTaxData result)
{
    if (!result.CredentialsIssuedByApi || string.IsNullOrWhiteSpace(result.Token) || string.IsNullOrWhiteSpace(result.Sign) || result.ExpirationTime is null)
    {
        return;
    ...
    constanciaCredentials = new ErpCredentialSnapshot(result.Token, result.Sign, result.ExpirationTime.Value);
    logger.LogInformation(
        "{Scenario}: la API emitio credenciales Constancia WSAA (expiran {Expiration:o}); el consumer las reutilizara.",
        scenario,
        constanciaCredentials.Expiration);
}
...
try
{
    var (constanciaToken, constanciaSign) = ApplyConstanciaCredentials();
    var persona = await constanciaService.GetPersonaAsync(
        cuit: receiverCuit,
        correlationId: "ws-constancia-test",
        token: constanciaToken,
        sign: constanciaSign);

    CaptureConstanciaCredentialsFromResult("WS-Constancia-GetPersona", persona);
